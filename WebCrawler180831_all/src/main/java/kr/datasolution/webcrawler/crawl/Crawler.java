...
public class Crawler {
	public static ArrayList<DbData> addCrawlDataToList(JsonData jsonData) throws Exception {
		...
		String title = "", date = "", content = null, contentUrl = "", webFileName = null, created = "";
	...
	public static ArrayList<DbData> addCrawlWorldBankDataToList(JsonData jsonData) throws Exception {
		System.out.println("Start addCrawlWorldBankDataToList =====");
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		SimpleDateFormat sdfUs = new SimpleDateFormat("yyyy-MM", Locale.US);
		
		String crawledTime = sdf.format(new Date()); // 메소드 시작 시간
		String title = "", date = "", contentUrl = "", webFileName = "";
		String nextPageAddParam = ""; // next page parameter 정보
		String noticeAddUrl = ""; // notice url의 추가 정보
		...
		for (int i = 0; i < 2; i++) {
			...
			if (i != 0) {
				// 다음 페이지의 url 파라미터 만들기
				nextPageAddParam = jsonData.getNextPageParam().get(0) + (i * 3); // ?offset=숫자
			...
			for (Element notice : notices) {
				...
				if (!notice.attr("data-href").equals("#")) {
					noticeAddUrl = notice.attr("data-href");
					// 맨 앞에 /가 있을 시 제거, url에 마지막에 /가 붙기에
					if (noticeAddUrl.charAt(0) == '/') { noticeAddUrl = noticeAddUrl.substring(1); }
					...
					crawlData = new CrawlData(title, sdf.format(sdfUs.parse(date)), null, crawledTime, url + noticeAddUrl);
		...
		System.out.println("End addCrawlWorldBankDataToList =====");
		return dbDatas;
	}

	public static ArrayList<DbData> addCrawlUNNewsDataToList(JsonData jsonData) throws Exception {
		System.out.println("Start addCrawlUNNewsDataToList =====");
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		SimpleDateFormat sdfUs = new SimpleDateFormat("dd MMMM yyyy", Locale.US);
		
		String crawledTime = sdf.format(new Date()); // 메소드 시작 시간
		String title = "", date = "", content = "";
		String noticeAddUrl = null; // notice url의 추가 정보
		String nextPageAddParam = null; // next page parameter 정보
		...
		for (int i = 0; i < 2; i++) {
			...
			if (i != 0) {
				// 다음 페이지의 url 파라미터 만들기
				nextPageAddParam = jsonData.getNextPageParam().get(0) + (i + 1);
			...
			for (Element notice : notices) {
				...
				if (!notice.attr("href").equals("#")) {
					noticeAddUrl = notice.attr("href");
					// 맨 앞에 /가 있을 시 제거, url에 마지막에 /가 붙기에
					if (noticeAddUrl.charAt(0) == '/') {
						noticeAddUrl = noticeAddUrl.substring(1);
					}

					// System.out.println(url + noticeAddUrl);
					dataDoc = Jsoup.connect(url + noticeAddUrl).get();

					title = dataDoc.select(crawlDataTag.getTitle()).text();
					date = dataDoc.select(crawlDataTag.getDate()).text();

					for (String tmp : crawlDataTag.getContent()) {
						content += dataDoc.select(tmp).text();
					}
					
					crawlData = new CrawlData(title, sdf.format(sdfUs.parse(date)), content, crawledTime, url + noticeAddUrl);
					
					dbData = DbRelatedDataInputer.addToDbData(crawlData, jsonData, null); // dbData객체에 필요한 데이터 저장
					
					dbDatas.add(dbData); // 데이터 담기
		...
		System.out.println("End addCrawlUNNewsDataToList =====");
		return dbDatas;
	}
}
